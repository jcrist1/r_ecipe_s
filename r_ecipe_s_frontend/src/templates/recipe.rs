...
pub fn recipe_page() -> View<G> {
    ...
    let id = RECIPE_ID.lock().unwrap().id;
    if G::IS_BROWSER {
        ...
        //            }).expect("err").unwrap();
                    let recipe = Recipe { name: format!("Hello: {}", id), ingredients: vec![], description: "Do stuff".to_string(), liked: None};
